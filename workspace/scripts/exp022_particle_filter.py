#!/usr/bin/env python3
"""exp022: Particle Filter GR-typewell tracker — honest 773-well CV (HEAVY).

参照ノートブック(ajayrao43/biohack44)の run_particle_filter + run_pf_lik_ensemble を
忠実移植(128 seed × 500 粒子, 尤度加重)。全773 train well で正直なリークなしCVを算出。

状態: pos = TVT + Z（Zは既知）, rate = dpos/dMD。MDに沿って滑らかに伝播し、
観測GRと typewell GR(pos-Z) の尤度で粒子を再重み付け(systematic resampling)。
128 seed を尤度加重平均。GR欠損は well全体を補間してから使用。完全leak-free。

**重い実験(>20分)**: well単位に ProcessPoolExecutor で並列化(全773・フル設定)。
"""
...
def _process_well(p):
    """1 well を 128 seed 尤度加重アンサンブル。(wid, pred[n])。"""
...
def main() -> None:
    ...
    print(f"[{EXP_ID}] Particle Filter — 773-well CV  (workers={N_WORKERS}, seeds={N_SEEDS}, particles={N_PARTICLES})")

    print("TRAIN 773 wells (full PF) ...", flush=True)
    ...
    print(f"  PF CV = {cv:.6f}   anchor = {anc:.6f}")
    ...
    print(f"  PF が anchor に勝つ well: {n_beat}/{len(well)}")

    print("TEST submission (full PF) ...", flush=True)
    test_preds = run_split("data/processed/test_base_v001.parquet",
    ...
    for wid, g in tp.groupby("well_id"):
        ...
        print(f"    test {wid}: PF vs train-truth RMSE = {test_well_rmse[wid]:.4f}")

    result = {
        "exp_id": EXP_ID, "created_at": now_jst(), "status": "completed",
        "method": f"Particle Filter ({N_SEEDS}seed x {N_PARTICLES}part, lik-weighted scale={SCALE}), leak-free",
        "cv_rmse": cv, "anchor_rmse": anc, "n_wells": int(len(well)),
        ...
        "notes": "Faithful PF port. State pos=TVT+Z, smooth motion along MD, GR likelihood vs typewell. "
                 "Full-well GR interpolation. ProcessPoolExecutor over wells.",
    }
    write_json(OUT_DIR / "result.json", result)

    twr = "\n".join(f"| {k} | {v:.4f} |" for k, v in test_well_rmse.items())
    (OUT_DIR / "notes.md").write_text(f"""# {EXP_ID} — Particle Filter GR-typewell トラッカー

## 手法
参照の run_particle_filter を忠実移植。状態 pos=TVT+Z をMD沿いに滑らか伝播、
観測GR vs typewell GR(pos-Z) の尤度で粒子再重み付け。{N_SEEDS} seed × {N_PARTICLES} 粒子, 尤度加重(scale={SCALE})。
...
PF が anchor に勝つ well: {n_beat}/{len(well)}
