#!/usr/bin/env python3
"""exp030b: Multi-typewell Particle Filter (vectorized) — 高速版

exp030(非ベクトル化)は遅すぎた(ETA 17h)。exp027のベクトル化PFを基盤に、
各wellでK=3 typewell候補それぞれにvectorized PF実行→ 候補log_likで加重平均。

各候補は S=32 seeds × N=500 particles を (S,N) 配列で一括計算。
3 × 32 = 96 seed相当 vs 元exp022の128 seed × 1typewell。
"""
...
EXP_ID = "exp030b_multi_tw_vec"
OUT_DIR = ROOT / "experiments" / EXP_ID
OUT_DIR.mkdir(parents=True, exist_ok=True)

S = 32          # seeds per candidate
N = 500         # particles
K_CAND = 3      # typewell candidates per well
SCALE = 8.0
INIT_SPREAD = 4.0   # tuned from exp025
PN = 0.01
VN = 0.002
MOM = 0.998
RP = 0.1
...
def pf_well_vec(tw_tvt, tw_gr, md_v, z_v, gr_v, gs, ir, last_tvt, last_Z, last_MD, seed=0):
    """1 typewell候補に対し vectorized PF (S,N) を実行。
    return (pred[n], best_log_lik)"""
    n = len(md_v)
    if n == 0:
    ...
    for i in range(n):
        ...
        prev_MD = md_v[i]
    # seed-level likelihood-weighted avg within this candidate
...
def main():
    t0 = time.time()
    print(f"[{EXP_ID}] multi-typewell vec PF  workers={N_WORKERS} S={S} N={N} K={K_CAND}")
    ...
    print(f"\n  multi-typewell vec PF CV = {cv:.6f}   anchor = {anc:.6f}")
    ...
    try:
        ...
        print(f"\n  vs exp022: better={n_better}, worse={n_worse}, "
              f"median delta={improvement.median():+.3f}, mean={improvement.mean():+.3f}")
    ...
    print("  test wells RMSE vs train truth:")
