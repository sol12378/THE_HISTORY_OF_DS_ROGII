# %% markdown 1
# # ROGII Dual Pipeline v17 NoPF Audit
# 
# ## Version 17: noPF conservative audit
# 
# Version 17 is the interpretability audit for the final set of probes. It disables the PF sidecar inside the Gold overlay while keeping the conservative profile and sparse MultiCut schedule.
# 
# This is not expected to be the most aggressive leaderboard attempt. Its value is explanatory: it separates the visible-prefix gate from the extra PF candidates and makes the public notebook easier to reason about when comparing the v14-v16 results.
# 
# ## How to read this notebook
# 
# This notebook is built as a reproducible experiment log, not just a `submission.csv` factory. The code has four main ideas:
# 
# 1. Build a SP45-style physical reconstruction branch, then polish it with a robust low-order projection.
# 2. Build an independent Fleongg-style LikPF / pretrained tabular branch.
# 3. Blend those two branches only after checking id alignment and finite values.
# 4. Apply a guarded Gold visible-prefix overlay at the end, so the final move is justified by each test well's own visible prefix.
# 
# That order matters. The Gold overlay is intentionally last because it should correct a strong public blend, not replace the whole modeling stack.
# 
# ## Recent version log
# 
# - **v7**: imported the newest public Gold visible-prefix calibration overlay with the conservative profile. This is the current best direction from the recent public run.
# - **v8**: tested the balanced profile. It allowed more movement, but the leaderboard result was worse than v7.
# - **v9**: tested the aggressive profile. This bracketed the high-risk side and showed that looser gates can fail badly.
# - **v10**: disabled the PF sidecar inside the Gold overlay. This isolated whether the improvement came from the gate or from PF candidates.
# - **v11**: increased prefix cut coverage and PF seeds to reduce selector noise. It was useful as a design probe, but too expensive for the CPU run.
# - **v12**: returns to the conservative profile, keeps a light MultiCut audit, removes the extra seed cost, and restores GPU for the upstream pipeline.
# - **v13**: keeps the conservative checkpoint and turns the notebook into a reader-first audit of the full pipeline.
# - **v14**: reruns the conservative Gold profile as the documented score anchor for the final public series.
# - **v15**: keeps the conservative gates but tests a sparse three-cut prefix audit without extra PF seed cost.
# - **v16**: uses the same conservative gates with a wider four-cut prefix schedule to test where the useful visible evidence appears.
# - **v17**: disables the PF sidecar inside the Gold overlay, giving a cleaner audit of the visible-prefix gate itself.
# 
# ## What changed in this commit
# 
# Active variant: **V17**.
# 
# Gold environment overrides:
# 
# - `ROGII_GOLD_PROFILE=conservative`
# - `ROGII_GOLD_INCLUDE_PF=0`
# - `ROGII_GOLD_CUT_FRACS=0.60,0.75,0.90`
# 
# ## Why this belongs in the public notebook
# 
# The useful part of this competition was not a single magic model. The public notebooks converged by comparing several independent geological interpretations and then being strict about when a final correction is allowed to move the curve. This version keeps that search path visible.
# 
# Each version changes one interpretable lever in the final Gold overlay: profile strictness, PF sidecar usage, prefix cut coverage, seed cost, or documentation/audit clarity. That makes the leaderboard experiments easier to audit and gives readers a concrete explanation for why a score moved.
# 
# Public work referenced: `baidalinadilzhan/rogii-lb-7-201`, `bernubritz/rogii-lb7295-public-rebuild`, `curvecowboy/rogii-lb7201-public-gold-conservative`, `fleongg/fle3n-rogii-v5`, and the earlier dual-pipeline forks.
# 

# %% code 2
import os
os.environ["ROGII_GOLD_PROFILE"] = "conservative"
os.environ["ROGII_GOLD_INCLUDE_PF"] = "0"
os.environ["ROGII_GOLD_CUT_FRACS"] = "0.60,0.75,0.90"
print("Gold overlay env:", {k: os.environ.get(k) for k in sorted(os.environ) if k.startswith("ROGII_GOLD_")})
...
# %% markdown 19
# ## Stage A: selector / physical reconstruction branch
# 
# This branch builds the SP45-style physical candidate: contact-based TVT reconstruction, particle-filter candidates, and robust projection in the `U = TVT + Z` frame.
# 
# The point is not that this branch is always best by itself. Its value is error diversity. A physically plausible branch can disagree with the LikPF branch exactly where a single tracker is likely drifting, which makes the later blend and Gold overlay more informative.
# 
...
# %% markdown 25
# ## Stage B: Fleongg LikPF branch and dual blend
# 
# The Fleongg branch supplies an independent likelihood-weighted particle-filter / pretrained tabular interpretation. This notebook keeps it separate until both candidate submissions are written and checked.
# 
# The dual blend is the anchor for the final notebook. It gives the Gold overlay something strong and stable to correct, instead of asking the overlay to solve the whole geology problem from scratch.
# 
...
# %% markdown 40
# ## Stage C: guarded overlap verification
# 
# The overlap logic is deliberately conservative. Duplicate-looking wells are not blindly copied from train to test: the code checks visible-prefix consistency, MD alignment, and finite outputs before allowing an override.
# 
# This section is important for reproducibility. It explains why the notebook can use public overlap evidence while avoiding the easy failure mode where a train-copy shortcut looks good on visible wells but breaks on hidden reruns.
# 
...
# %% markdown 43
# ## Stage D: Version 17: noPF conservative audit
# 
# Version 17 is the interpretability audit for the final set of probes. It disables the PF sidecar inside the Gold overlay while keeping the conservative profile and sparse MultiCut schedule.
# 
# This is not expected to be the most aggressive leaderboard attempt. Its value is explanatory: it separates the visible-prefix gate from the extra PF candidates and makes the public notebook easier to reason about when comparing the v14-v16 results.
# 
# The following cell performs the final overlay and writes the scored `submission.csv`. It also writes audit artifacts so the final move can be inspected after the run.
# 
