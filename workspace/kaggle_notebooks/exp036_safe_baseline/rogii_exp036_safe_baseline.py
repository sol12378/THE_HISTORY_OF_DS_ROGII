"""ROGII exp036 — Safe baseline (leak-free): same components as exp034 but W_LEAK=0.

Final 2-submission 保守枠候補。Private shake-up 防御の基準線。

Original exp034 docstring:

これまでの実験結果を踏まえた最強構成 + leak lookup の hybrid 0.5/0.5.
...
W_PF_PHYS = 0.15
W_GEOM = 0.05
# Hybrid weight: 0.5 model + 0.5 leak (testing transferability with risk hedge)
HYBRID_W_MODEL = 1.0
HYBRID_W_LEAK = 0.0
SMOOTH_W = 101
