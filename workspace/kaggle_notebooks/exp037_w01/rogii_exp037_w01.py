"""ROGII exp037 w01 — Hybrid (PF/multi-tw/physical-lik/geom blend) × leak lookup.

これまでの実験結果を踏まえた最強構成 + leak lookup の hybrid 0.9/0.1 (leak weight 0.1).
...
Hybrid:
  final = 0.9 × model_blend + 0.1 × leak_lookup
  → well 内 row順 mean 平滑 (w=101)

Leak risk hedge:
- w01: leak weight 0.1 (conservative, prioritize model)
...
W_PF_PHYS = 0.15
W_GEOM = 0.05
# Hybrid weight: 0.9 model + 0.1 leak (testing transferability with risk hedge)
HYBRID_W_MODEL = 0.9
HYBRID_W_LEAK = 0.1
SMOOTH_W = 101
